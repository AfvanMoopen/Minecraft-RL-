import gym
import mine.minePython as minePython
import random
import time
import numpy as np
from enum import Enum

CLIENT_PORT = 9000                  # mine port
...
class mineEnv(gym.Env):
    ...
    run Project mine 0.36.0 Python API for solving
    maze.

    init parameters
    ---------------
    xml : str (required)
        Mission setting (XML string) used in Project mine.
    width : int (required)
    ...
    def __init__(self,
        ...
        # Create AgentHost
        self.agent_host = minePython.AgentHost()
        # Create MissionRecordSpec
        self.my_mission_record = minePython.MissionRecordSpec()
        self.my_mission_record.recordRewards()
        self.my_mission_record.recordObservations()
        # Create ClientPool
        self.pool = minePython.ClientPool()
        client_info = minePython.ClientInfo('127.0.0.1', CLIENT_PORT)
    ...
    def reset(self):
        ...
        my_mission = minePython.MissionSpec(xml,True)
        # Start mission
